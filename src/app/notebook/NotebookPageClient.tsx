...
export function NotebookPageClient({ initialGroups }: NotebookPageClientProps) {
  ...
  async function handleCreate(
    ...
    setGroups((prev) => [...prev, { ...group!, wordCount: 0 }]);
      ...
      <AppLayout breadcrumb={breadcrumb} initialGroups={initialGroups}>
        <div className="flex flex-col gap-2 max-w-2xl">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="flex items-stretch gap-0 rounded-lg border bg-card overflow-hidden">
  ...
  return (
    <>
      <AppLayout breadcrumb={breadcrumb} headerActions={headerActions} initialGroups={initialGroups}>
        {groups.length === 0 ? (
