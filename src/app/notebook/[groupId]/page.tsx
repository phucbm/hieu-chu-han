...
export default function GroupDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    const [allGroups, words] = await Promise.all([
      getGroups(),
    ...
    if (found?.type === "lyrics") {
      const lyricsData = await getLyrics(groupId);
        ...
        <header className="flex h-16 shrink-0 items-center gap-2 px-4">
          ...
          <Breadcrumb className="flex-1 min-w-0">
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage className="truncate">{group.title}</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </header>

        <div className="flex flex-1 flex-col p-4 pt-0 gap-4 max-w-2xl">

          {/* Lyrics panel — only for lyrics groups */}
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2">
              <h2 className="text-sm font-medium flex-1">
                Từ vựng
                {groupWords.length > 0 && (
                  <span className="text-muted-foreground font-normal ml-1.5">({groupWords.length})</span>
                )}
              </h2>
            </div>
            ...
            >
              <Input
                ...
                disabled={adding}
              />
              <Button type="submit" size="sm" className="h-8" disabled={adding || !addInput.trim()}>
